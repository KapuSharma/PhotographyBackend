...
router.patch("/:id", async (req, res) => {
  try {
    ...
    const updated = await updateOwned(getPrisma().galleryImage, req.params.id, req.user.clientId, data);
...
router.delete("/:id", async (req, res) => {
  const ok = await deleteOwned(getPrisma().galleryImage, req.params.id, req.user.clientId);
